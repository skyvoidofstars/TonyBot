...
def setup_events(bot: commands.Bot):
    ...
    async def on_app_command_error(interaction: discord.Interaction, error):
        ...
        if isinstance(actual_error, commands.MissingAnyRole):
            await bot.get_guild(interaction.guild_id).get_channel(
                interaction.channel_id
            ).send(
                f"{interaction.user.mention} Você não tem permissão para usar o comando {interaction.command.name}!"
            )
            ...
            await bot.get_guild(interaction.guild.id).get_channel(LogChannel).send(
