...
def setup_commands(bot: commands.Bot):
    ...
    @app_commands.describe(data_limite="Ex: 31/05/2025")
    async def resumo(interaction: discord.Interaction, data_limite: str):
        try:
            ...
            await interaction.response.send_message(
                content=f"Falha ao extrair data de `{data_limite}`, verifique e tente novamente"
            )
            return
        ...
        dates_interval: str = (
            f"{lower_limit_date.strftime(format='%d/%m')} - {upper_limit_date.strftime(format='%d/%m')}"
        )
