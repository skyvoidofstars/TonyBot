...
async def _update_seizure_messages(bot: commands.Bot, refund_id: int):
    ...
    for _message in _seizure_messages:
        try:
            _fetched_message: discord.Message = await bot.get_channel(seizure_channel_id).fetch_message(_message[0])
...
class ApproveRefundView(ui.View):
    ...
    async def approve_button(self, interaction: discord.Interaction, button: ui.Button):
        ...
        total_value: int = (
            ...
            .filter(Seizure.status == 'CRIADO', Seizure.created_at <= self.upper_limit_date)
